...
*	Scope	:	public
* 
*	Purpose	:	To detect collision for octrees
* 
*	returns	:	bool
*/

bool COctree::OctreeCollision(glm::vec3 testpoint)
{
	/*cout << m_dMinX << " " << m_dMinY << " " << m_dMinZ
		<< " " << m_dMaxX << " " << m_dMaxY << " " << m_dMaxZ << endl;*/
	...
	else
	{
		...
		{	
			if (m_pobChildren[i] != nullptr)
			{
				if (m_dMinX <= testpoint.x && m_dMinY <= testpoint.y && 
					m_dMinZ <= testpoint.z && m_dMaxX >= testpoint.x && 
					m_dMaxY >= testpoint.y && m_dMaxZ >= testpoint.z)
				{
					/*cout << m_iLevel << " " << m_dMinX << " " << m_dMinY << " " << m_dMinZ
						<< " " << m_dMaxX << " " << m_dMaxY << " " << m_dMaxZ << endl;*/
					result = result || m_pobChildren[i]->OctreeCollision(testpoint);
				}
			}
		}
		return result;
